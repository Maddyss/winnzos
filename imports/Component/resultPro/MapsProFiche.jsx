...
export default class MapsProFiche extends Tracker.Component {
    constructor(props){
        super(props);
        this.state = {
            pos: {position:{'lat': parseFloat(48.862725),'lng': parseFloat(2.287592000000018)}}
        }
        var IdFichePro = String(this.props.pos);
        this.subscribe('fichePro.id', IdFichePro);
        this.autorun(() => {
            var fichePro = FichePro.findOne(IdFichePro);
            this.setState({
                pos: fichePro
            })
        });
